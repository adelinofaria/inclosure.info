...
class Tools
{
	...
	*/
	static public function redirect($url, $baseUri = CONTENT_URL)
	{
...
//	}
	...
	static public function setCurrency()
	{
		...
		$currency = new Currency(intval(Configuration::get('CURRENCY_DEFAULT')));
	...
	*/
	static public function displayPrice($price, $currency, $no_utf8 = false, $convert = true)
	{
		...
		$c_decimals = (is_array($currency) ? intval($currency['decimals']) : intval($currency->decimals)) * intval(Configuration::get('PRICE_DISPLAY_PRECISION'));
	...
	*/
	...
	{
		$c_id = (is_array($currency) ? $currency['id'] : $currency->id);
		$c_rate = (is_array($currency) ? $currency['conversion_rate'] : $currency->conversion_rate);
		if ($c_id != intval(Configuration::get('CURRENCY_DEFAULT')))
			$price *= $c_rate;
		return $price;
	}
	...
	* @return string Date
	*/
	static public function dateFormat($params, $lang)
	{
		return Tools::displayDate($params['date'], $lang, $params['full']);
	}
	...
	*/
	static public function displayError($string = 'Hack attempt', $htmlentities = true)
	{
		...
		if (!is_array($_ERRORS))
			return str_replace('"', '&quot;', ($htmlentities ? htmlentities($string, ENT_COMPAT, 'UTF-8') : $string)  );
	}
	...
	*/
	...
	{
		return md5(pSQL(COOKIE_KEY.$passwd));
	}
...
//	}
	...
	*/
	...
	{
		$row = MySQL::getInstance()->getRow('SELECT COUNT(`id`) AS total FROM `category`');
	...
	*/
	...
	{
		$row = MySQL::getInstance()->getRow('SELECT COUNT(`id`) AS total FROM `products`');
		return intval($row['total']);
	}
	...
	*/
	...
	{
		$row = MySQL::getInstance()->getRow('SELECT COUNT(`id`) AS total FROM `customer`');
		return intval($row['total']);
	}
	...
	*/
	...
	{
		$row = MySQL::getInstance()->getRow('SELECT COUNT(`id`) AS total FROM `order`');
		return intval($row['total']);
	}
	...
	*/
	...
	static public function getTimezones($select = false)
	{
		...
		if ($select)
		{
			...
			if (!$_cache)
			{
				$tmz = MySQL::getInstance()->getRow('SELECT `name` FROM timezone WHERE id = '.intval($select));
				$_cache = $tmz['name'];
			}
			return $_cache;
		}
		...
		// Multiple select
		$tmz = Db::getInstance()->s('SELECT * FROM timezone');
		$tab = array();
		foreach ($tmz as $timezone)
			$tab[$timezone['id']] = str_replace('_', ' ', $timezone['name']);
		return $tab;
	}
